...
// int[] idx = IntStream.range(0, arr.length).filter(i -> arr[i] ==
// 2).toArray(); // arr[i] == 2 인 i 만 배열요소로 남기기
// if(idx.length == 0) return new int[]{-1};
// return IntStream.rangeClosed(idx[0], idx[idx.length-1]).map(i ->
// arr[i]).toArray(); // 배열 arr 에서 해당 인덱스의 값을 가져와 스트림으로 매핑 -> 매핑된 값을 정수배열로 반환
...
// int[] answer = Arrays.copyOfRange(arr, result.get(0),
